...
function mapProgramme(raw: string | null): 'Half Day' | 'Full Day' | 'Half Day + Enrichment' {
  if (!raw) return 'Half Day';
  const s = raw.toLowerCase();
  if (s.includes('full day') || s.includes('全天')) return 'Full Day';
  if (s.includes('enrichment') || s.includes('6 hour') || s.includes('6hour')) return 'Half Day + Enrichment';
  return 'Half Day';
}
