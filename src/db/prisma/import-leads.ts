...
function resolveStatus(row: any[]): { status: string; lostReason?: string } {
  ...
  if (enrolled)    return { status: 'ENROLLED' };
  if (declined)    return { status: 'LOST', lostReason: reason };
  if (rejected)    return { status: 'LOST', lostReason: reason };
  if (didntAttend) return { status: 'LOST', lostReason: "Didn't attend the enquiry" };
  if (attended)    return { status: 'FOLLOW_UP' };
  if (hasAppt)     return { status: 'APPOINTMENT_BOOKED' };
  return { status: 'NEW' };
}
...
async function main() {
  ...
  for (let i = 1; i < rows.length; i++) {
    const row = rows[i];
    // Skip empty rows
    if (!row || !row[0] || !row[1] || !row[2] || !row[3]) { skipped++; continue; }
    ...
    let appointmentEnd: Date | undefined;
