...
async function main() {
  ...
  // Helpers
  const d  = (y: number, m: number, day: number) => new Date(y, m - 1, day);       // childDob
  const at = (y: number, m: number, day: number) => new Date(y, m - 1, day, 10);   // submittedAt
  const appt = (y: number, m: number, day: number) => new Date(y, m - 1, day, 10, 0); // appointmentStart/End
  ...
  const packageData = packageYears.flatMap((year) =>
